...
abstract class BaseQuestion extends BaseObject  implements Persistent {
  ...
  const PEER = 'QuestionPeer';
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(QuestionPeer::INTERESTED_USERS))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->interested_users = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->stripped_title = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Question has previously been saved, it will retrieve
	 * related Interests from storage. If this Question is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collInterests === null) {
			if ($this->isNew()) {
			   $this->collInterests = array();
				...
				$criteria->add(InterestPeer::QUESTION_ID, $this->id);
	 ...
	 * actually need in Question.
	 */
	public function getInterestsJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(QuestionPeer::DATABASE_NAME);
		}
		...
		if ($this->collInterests === null) {
			if ($this->isNew()) {
				...
				$this->collInterests = InterestPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
